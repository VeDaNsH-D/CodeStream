...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const ui = {
        ...
        themeToggleBtn: document.getElementById('theme-toggle-btn'), themeIconSun: document.getElementById('theme-icon-sun'), themeIconMoon: document.getElementById('theme-icon-moon'),
    };
    ...
    const setupSidebarToggle = () => { const toggle = () => { ui.sidebar.classList.toggle('-translate-x-full'); ui.sidebarOverlay.classList.toggle('hidden'); }; ui.sidebarToggleBtn.addEventListener('click', toggle); ui.sidebarOverlay.addEventListener('click', toggle); };
    ...
    const setupResizer = () => { let isResizing = false; ui.resizer.addEventListener('mousedown', () => { isResizing = true; document.body.style.cursor = 'ew-resize'; document.body.style.userSelect = 'none'; }); document.addEventListener('mousemove', (e) => { if (isResizing) { const totalWidth = window.innerWidth; const newLeftWidthPercent = (e.clientX / totalWidth) * 100; if (newLeftWidthPercent > 20 && newLeftWidthPercent < 80) { ui.editorPane.style.width = `${newLeftWidthPercent}%`; ui.rightPane.style.width = `${100 - newLeftWidthPercent}%`; } } }); document.addEventListener('mouseup', () => { isResizing = false; document.body.style.cursor = 'default'; document.body.style.userSelect = 'auto'; }); };
    async function handleJoinAttempt(isCreating) { const username = ui.usernameInput.value.trim(); if (!username) return alert('Please enter your name.'); const roomId = isCreating ? `cs-${Math.random().toString(36).substr(2, 9)}` : ui.roomIdInput.value.trim(); if (!roomId) return alert('Please enter a Room ID.'); await initializeMedia(); state.username = username; state.currentRoomId = roomId; ui.entryModal.classList.add('hidden'); ui.app.classList.remove('hidden'); ui.app.classList.add('flex'); window.history.pushState(null, '', `?room=${roomId}`); if (socket.connected) socket.emit('join-room', { roomId: state.currentRoomId, username: state.username }); }
    const renderFileExplorer = () => { ui.fileExplorer.innerHTML = Object.keys(state.files).sort().map(path => `<div class="flex justify-between items-center group p-1 rounded hover:bg-black/10 dark:hover:bg-white/10 cursor-pointer" data-path="${path}"><span class="file-name text-gray-700 dark:text-gray-300 group-hover:text-black dark:group-hover:text-white">${path}</span><div class="hidden group-hover:flex items-center"><button class="rename-file-btn text-xs text-gray-500 dark:text-gray-400 hover:text-black dark:hover:text-white mr-1" data-path="${path}">✏️</button><button class="delete-file-btn text-xs text-gray-500 dark:text-gray-400 hover:text-black dark:hover:text-white" data-path="${path}">🗑️</button></div></div>`).join(''); ui.fileExplorer.querySelectorAll('[data-path]').forEach(el => el.addEventListener('click', (e) => !e.target.closest('button') && openTab(el.dataset.path))); ui.fileExplorer.querySelectorAll('.rename-file-btn').forEach(btn => btn.addEventListener('click', renameFile)); ui.fileExplorer.querySelectorAll('.delete-file-btn').forEach(btn => btn.addEventListener('click', deleteFile)); };
    const renderParticipants = () => { const participants = Object.values(state.participants); const avatar = (p) => `<div class="w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center text-sm font-bold" style="background-color: ${p.color}; color: #fff;">${p.username.charAt(0).toUpperCase()}</div>`; ui.participantList.innerHTML = participants.map(p => `<div class="flex items-center space-x-3 p-2 rounded-md hover:bg-black/5 dark:hover:bg-white/5">${avatar(p)}<span class="font-medium">${p.username} ${p.id === state.currentUser?.id ? '<span class="text-xs text-blue-400">(You)</span>' : ''}</span></div>`).join(''); };
    const renderTabs = () => { ui.tabsContainer.innerHTML = state.openTabs.map(path => `<div class="tab flex items-center px-4 py-2 border-r border-gray-300 dark:border-gray-700/50 cursor-pointer ${path === state.activeTab ? 'bg-white dark:bg-[#1e1e1e]' : 'bg-transparent hover:bg-gray-100 dark:hover:bg-[#333]'}" data-path="${path}"><span class="text-sm font-medium">${path}</span><button class="close-tab-btn ml-3 text-gray-500 hover:text-black dark:hover:text-white" data-path="${path}">×</button></div>`).join(''); ui.tabsContainer.querySelectorAll('.tab').forEach(tab => tab.addEventListener('click', (e) => !e.target.classList.contains('close-tab-btn') && switchTab(tab.dataset.path))); ui.tabsContainer.querySelectorAll('.close-tab-btn').forEach(btn => btn.addEventListener('click', (e) => { e.stopPropagation(); closeTab(btn.dataset.path); })); };
    ...
    function initializeEditor() { if (state.editorInstance) return; const editorDiv = document.getElementById('editor'); require.config({ paths: { 'vs': 'https://cdn.jsdelivr.net/npm/monaco-editor@0.45.0/min/vs' }}); require(['vs/editor/editor.main'], function () { state.editorInstance = monaco.editor.create(editorDiv, { value: "// Welcome!", language: 'javascript', theme: localStorage.getItem('theme') === 'light' ? 'vs' : 'vs-dark', automaticLayout: true, fontSize: 14, }); state.editorInstance.onDidChangeModelContent(e => { if (state.isProgrammaticChange) return; const newCode = state.editorInstance.getValue(); state.files[state.activeTab] = newCode; const PASTE_THRESHOLD = 50; const isPaste = e.changes.length === 1 && e.changes[0].text.length > PASTE_THRESHOLD; if (isPaste) socket.emit('large-paste'); socket.emit('code-change', { path: state.activeTab, newCode: newCode }); }); const firstFile = Object.keys(state.files)[0]; if (firstFile) openTab(firstFile); }); }
    ...
    socket.on('initial-sync', (data) => { const isFirstSync = !state.editorInstance; Object.assign(state, data); renderFileExplorer(); renderParticipants(); if (isFirstSync) initializeEditor(); });
    ...
    socket.on('receive-chat-message', ({ user, message }) => { const messageEl = document.createElement('div'); messageEl.innerHTML = `<div><b style="color:${user.color};">${user.username}:</b> <span class="text-gray-700 dark:text-gray-300">${message}</span></div>`; ui.chatMessages.appendChild(messageEl); ui.chatMessages.scrollTop = ui.chatMessages.scrollHeight; });
